...
use zk_onchain::vaults::server_vault::ServerVaultParams;
...
pub fn derive_server_vault(
    ...
    let address_seed = derive_address_seed(
        &[SERVER_VAULT.as_bytes(), authority.as_ref()],
        &zk_onchain::ID,
    );
...
async fn get_account_params(
    ...
    let server_vault = server_vault.unwrap_or(Pubkey::from(derive_server_vault(
        current_authority,
        address_merkle_context,
    )));
    let address_string = bs58::encode(server_vault).into_string();
...
async fn build_instructions(
    ...
    let mut init_vault_ix = program
        .request()
        .accounts(soda_accounts::InitializeServerVault {
            payer: payer.pubkey(),
        ...
        .args(soda_instructions::InitializeServerVault {
...
pub async fn initialize_server_vault(
    ...
    let params = get_account_params(current_authority.pubkey(), server_vault).await?;
...
async fn get_update_server_cpda_params(
    current_authority: Pubkey,
) -> Result<(
    u16,
    PackedMerkleContext,
    ...
    if let Some(compressed_account) = compressed_accounts.result {
        ...
        let server_vault_state =
            zk_onchain::vaults::server_vault::ServerVaultState::try_from_slice(&decoded_bytes)?;

        println!("Server Vault: {server_vault_state:#?}");
    }
...
async fn update_server_vault_instructions(
    config: &ClientConfig,
    payer: &Arc<Keypair>,
    current_authority: &Keypair,
    ...
    // upate-server-vault-instruction
    let mut update_server_vault_ix = program
        .request()
        .accounts(soda_accounts::UpdateServerVaultAuthority {
            payer: payer.pubkey(),
            self_program: zk_onchain::id(),
            service_signer: service_signer.pubkey(),
            current_authority: current_authority.pubkey(),
        ...
        .args(soda_instructions::UpdateServerVaultAuthority {
            proof: compressed_proof,
    ...
    instructions.extend(update_server_vault_ix);

    Ok(instructions)
}

// Main vault initialization function
pub async fn update_server_vault(rpc_client: RpcClient) -> Result<()> {
    let client_config = "client_config.ini";
    let config = load_cfg(&client_config.to_string()).unwrap();
    // Submit transaction
    ...
    // Get account parameters and proof
    let params = get_update_server_cpda_params(current_authority.pubkey()).await?;

    // Build instructions
    let instructions = update_server_vault_instructions(
        &config,
        &payer,
        &current_authority,
        &new_authority,
